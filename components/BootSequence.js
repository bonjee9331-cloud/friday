...
// ── CONFIGURE YOUR MUSIC ─────────────────────────────────────────────────
// Find the YouTube video ID for 'Tin Tin Out - Strings for Yasmin'
// e.g. if the URL is youtube.com/watch?v=ABC123, set MUSIC_ID = 'ABC123'
const MUSIC_ID    = 'XXswgVBbTjU';  
const MUSIC_START = 200;            // 3 min 20 sec
// ─────────────────────────────────────────────────────────────────────────
...
export default function BootSequence({ onComplete }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/news').then(r => r.json()).then(d => setNews(d.headlines || [])).catch(() => {});
    fetch('/api/briefing').then(r => r.json()).then(d => setBriefing(d.briefing || '')).catch(() => {});
  }, []);
  ...
  const speak = useCallback(async (text) => {
    ...
    try {
      ...
      src.start(0);
      src.onended = () => { setBriefingDone(true); ac.close(); };
    } catch {
      setBriefingDone(true);
    }
  }, []);

  // boot log text stream
  useEffect(() => {
    let i = 0;
    const tick = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // hex grid (fades in during phase 2)
      if (ph >= 1) {
        const HEX = 34;
      ...
      // FRIDAY title
      if (ph >= 1) {
        const titleAlpha = Math.min(1, (ph >= 1 ? 1 : 0));
        ctx.font = '300 ' + (ph >= 2 ? '36' : '48') + 'px monospace';
        ctx.letterSpacing = '18px';
        ctx.fillStyle = 'rgba(0,200,255,' + titleAlpha + ')';
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#000010', overflow: 'hidden', fontFamily: 'monospace' }}>
      <canvas ref={canvasRef} style={{ position: 'absolute', inset: 0 }} />

      {/* YouTube music player — hidden, plays Strings for Yasmin from 2:20 */}
      <iframe
        ref={ytRef}
        src={'https://www.youtube.com/embed/' + MUSIC_ID + '?autoplay=1&start=' + MUSIC_START + '&loop=1&playlist=' + MUSIC_ID}
        allow='autoplay'
        style={{ position: 'absolute', width: 1, height: 1, opacity: 0.01, top: 0, left: 0, pointerEvents: 'none' }}
        title='music'
      />

      {/* boot log — left side */}
      <div style={{
        position: 'absolute', top: 80, left: 40, zIndex: 10,
        color: '#00b4ff', fontSize: 11, letterSpacing: 2,
        lineHeight: 1.9, maxWidth: 340,
        opacity: phase >= 1 ? 1 : 0, transition: 'opacity 1s',
      }}>
        {logLines.map((line, i) => (
          <div key={i} style={{
            color: line.includes('NOMINAL') || line.includes('BEN') ? '#00ffaa' :
                   line.includes('OK') ? '#00c8ff' : 'rgba(0,180,255,0.7)',
      ...
      {/* data panels — slide in at phase 4 */}
      {showData && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 10, pointerEvents: 'none' }}>
          ...
          <div style={{
            ...
            {weather ? (
              <div>
                <div style={{ fontSize: 32, fontWeight: 100 }}>{weather.temp}°C</div>
                <div style={{ fontSize: 11, marginTop: 4, color: '#00eaff' }}>
                  {wmoDesc[weather.code] || weather.desc}
                </div>
      ...
      <style>{`
        ...
        @keyframes slideInUp { from { opacity: 0; transform: translateX(-50%) translateY(30px); } to { opacity: 1; transform: translateX(-50%) translateY(0); } }
