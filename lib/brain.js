...
// Ask Friday with an explicit system prompt (used by agent router)
export async function askFridayWithSystem({ userMessage, history = [], systemPrompt, maxTokens = 1500 }) {
  const messages = [
    ...history.map((m) => ({ role: m.role, content: m.content })),
    { role: 'user', content: userMessage }
  ];
  const primary = PRIMARY === 'openai'
    ? await callOpenAI({ system: systemPrompt, messages, maxTokens })
    : await callClaude({ system: systemPrompt, messages, maxTokens });
